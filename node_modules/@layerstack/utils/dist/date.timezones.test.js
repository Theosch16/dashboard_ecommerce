...
import { endOfInterval, formatDate, getDateFuncsByPeriodType, getMonthDaysByWeek, intervalOffset, localToUtcDate, startOfInterval, timeInterval, utcToLocalDate, } from './date.js';
...
describe.each(TIMEZONES)('TZ=%s', (timeZone) => {
    ...
    describe('utc intervals', () => {
        ...
        it('intervalOffset() shifts by whole UTC intervals', () => {
            ...
            expect(intervalOffset('utcDay', utcMidnight, -6).toISOString()).equal('2026-08-04T00:00:00.000Z');
            // `offset` shifts, it does not floor — the day of month is preserved.
            expect(intervalOffset('utcMonth', utcMidnight, 1).toISOString()).equal('2026-09-10T00:00:00.000Z');
        });
        it('offsets across a DST transition without shifting the time of day', () => {
            ...
            expect(intervalOffset('utcDay', beforeSpringForward, 7).toISOString()).equal('2026-03-12T00:00:00.000Z');
        });
        it.each([
            ...
            ['quarter', 'utcQuarter'],
            ['year', 'utcYear'],
        ])('timeInterval() resolves both %s and %s', (local, utc) => {
        ...
        it.each([
            ...
            'utcQuarter',
            'utcYear',
        ])('timeInterval(%s) is distinct from its local counterpart', (utc) => {
            ...
            const local = (utc.charAt(3).toLowerCase() + utc.slice(4));
        ...
        ])('%s floors to %s in every zone', (interval, expected) => {
    ...
    describe('{ utc: true }', () => {
        ...
        describe('formatDate()', () => {
            ...
            it('renders the UTC month/year', () => {
                ...
                expect(formatDate(new Date('2026-12-31T23:59:59.999Z'), PeriodType.CalendarYear, {
                    utc: true,
                })).equal('2026');
            });
        ...
        describe('getDateFuncsByPeriodType()', () => {
            ...
            it('floors a fiscal year on the UTC boundary', () => {
                const { start, end } = getDateFuncsByPeriodType(defaultLocale, PeriodType.FiscalYearOctober, { utc: true });
