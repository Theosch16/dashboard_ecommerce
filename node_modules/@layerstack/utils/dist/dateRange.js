import { getDateFuncsByPeriodType, intervalOffset, isLeapYear, isDateAfter, isDateBefore, startOfInterval, updatePeriodTypeWithWeekStartsOn, } from './date.js';
 ...
 */
export function getDateRangePresets(settings, periodType, options) {
    let now = new Date();
        ...
        case PeriodType.Day:
            ...
            const adjustForLeapYear = options?.referenceDate
                ? (isLeapYear(options?.referenceDate) &&
                    isDateAfter(options?.referenceDate, new Date(options?.referenceDate.getFullYear(), /*Feb*/ 1, 28))) ||
                    (isLeapYear(intervalOffset('year', options?.referenceDate, -1)) &&
                        isDateBefore(options?.referenceDate, new Date(options?.referenceDate.getFullYear(), /*Feb*/ 1, 29)))
                : false;
...
export function getPeriodComparisonOffset(settings, view, period, options) {
