...
describe('BarChart', () => {
    describe('faceted tooltip header', () => {
        ...
        async function headerAfterHover(container) {
            let header = '';
            await vi.waitFor(() => {
        ...
        it('should name the panel in front of the band', async () => {
            ...
            expect(await headerAfterHover(container)).toBe('Adelie · Torgersen');
        });
        it('should format the band value before joining the panel to it', async () => {
            ...
            const header = await headerAfterHover(container);
    ...
    describe('stacked bar rounding', () => {
        ...
        const isRounded = (el) => el.tagName === 'path' && /a[\d.]+,[\d.]+/.test(el.getAttribute('d') ?? '');
        ...
        it('should resolve a `fillOpacity` accessor against each row', async () => {
            ...
            const { container } = render(BarChart, {
                props: {
                    ...
                    y: 'value',
                    height: 300,
                    props: { bars: { fillOpacity: (d) => (d.value > 15 ? 1 : 0.25) } },
                },
            });
            const svg = container.querySelector('svg');
            await expect.element(svg).toBeInTheDocument();
            const opacities = [...container.querySelectorAll('.lc-bar')].map((el) => el.getAttribute('fill-opacity'));
    ...
    describe('legend `c` category toggle', () => {
        ...
        async function legendButtons(container) {
            // `Legend` is lazy-loaded inside `ChartChildren`
            let buttons = container.querySelectorAll('.lc-legend-swatch-button');
            await vi.waitFor(() => {
        ...
        const bars = (container) => Array.from(container.querySelectorAll('.lc-bar')).map((el) => ({
            fill: el.getAttribute('fill'),
            opacity: el.style.opacity || el.getAttribute('opacity') || '1',
        }));
        it('should hide a category\u2019s bars when its legend item is clicked', async () => {
            const { container } = render(BarChart, longDataProps);
    ...
    describe('long data stacking', () => {
        ...
        async function segments(container) {
        ...
        const band = (bars, i) => bars.slice(i * 2, i * 2 + 2).sort((a, b) => a.top - b.top);
        ...
        it('should list the band\u2019s rows in the tooltip when `c` stacks them', async () => {
            ...
            const { container } = render(BarChart, stackedProps);
        ...
        it('should stack each facet panel against its own rows', async () => {
            const { container } = render(BarChart, { ...stackedProps, fx: 'panel' });
            await vi.waitFor(() => expect(container.querySelectorAll('.lc-bar').length).toBe(4));
