...
describe('LineChart', () => {
    ...
    it('should colour the highlight point from `c`, like the line it sits on', async () => {
        ...
        await vi.waitFor(() => {
            tooltipRect.dispatchEvent(new PointerEvent('pointerenter', eventInit));
            tooltipRect.dispatchEvent(new PointerEvent('pointermove', eventInit));
            const fills = Array.from(document.querySelectorAll('.lc-highlight-point')).map((p) => p.getAttribute('fill'));
...
describe('legend `c` category toggle', () => {
    ...
    const splines = (container) => Array.from(container.querySelectorAll('.lc-path')).map((el) => ({
    ...
    it('should fade the other lines while a legend item is hovered', async () => {
        const { container } = render(LineChart, longDataProps);
...
describe('legend over a colour a mark implied', () => {
    ...
    it('lists the categories, not the rows', async () => {
        const { container } = render(LineChart, { props: impliedProps });
        await vi.waitFor(() => {
            const labels = Array.from(container.querySelectorAll('.lc-legend-swatch-label')).map((el) => el.textContent?.trim());
...
describe('LineChart seriesLayout', () => {
    ...
    it('overlaps several series rather than inferring a stack', async () => {
        ...
        let ctx;
        render(LineChartCustomMarks, { ...seriesProps, oncontext: (c) => (ctx = c) });
