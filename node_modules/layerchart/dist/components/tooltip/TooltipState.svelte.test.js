...
async function renderChart(chartProps = {}) {
    let ctx = null;
...
describe('TooltipState', () => {
    ...
    describe('show(event, data)', () => {
        ...
        function pointerEventAt(ctx, x, y) {
        ...
        it('still resolves data from a pointer event', async () => {
            const ctx = await renderChart({ tooltipContext: { mode: 'bisect-x' } });
            const e = pointerEventAt(ctx, ctx.padding.left + ctx.xScale(data[2].date), ctx.padding.top + ctx.yScale(data[2].value));
            ctx.tooltip.show(e);
            expect(ctx.tooltip.data).toEqual(data[2]);
        });
