...
describe('Spline', () => {
    ...
    describe('class', () => {
        it('resolves per line when given a function', async () => {
            render(TestHarness, {
                ...
                componentProps: {
                    z: 'group',
                    class: (d) => (d.group === 'a' ? 'first-line' : 'second-line'),
                },
            });
            await expect.poll(() => paths().length).toBe(2);
    ...
    describe('style-split runs', () => {
        ...
        const dashOn = (d) => (d.gap ? 'dashed' : 'solid');
        ...
        it('keeps the `c` color on every run when only `class` splits the line', async () => {
            render(TestHarness, {
                ...
                componentProps: { class: (d) => (d.value % 20 === 0 ? 'even' : 'odd') },
            });
            ...
            expect(paths().every((p) => ['rgb(255, 0, 0)', 'rgb(0, 0, 255)'].includes(p.getAttribute('stroke')))).toBe(true);
        ...
        it('lets a `stroke` function color runs individually', async () => {
            ...
            render(TestHarness, {
                ...
                componentProps: {
                    seriesKey: 'value',
                    stroke: (d) => (d.gap ? 'rgb(255, 0, 0)' : 'rgb(0, 0, 255)'),
                },
            });
            await expect.poll(() => paths().length).toBe(3);
            expect(paths().map((p) => p.getAttribute('stroke'))).toEqual([
