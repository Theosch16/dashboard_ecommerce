...
import { createMotion, createPathMotionMap, extractTweenConfig, } from '../../utils/motion.svelte.js';
import { colorPropDataKey, resolveColorProp, resolveStyleProp } from '../../utils/dataProp.js';
...
export class SplineState {
    ...
    constructor(getProps) {
        ...
        this.#tweenState = createMotion(this.#defaultPathData(), () => this.d, tween ? { type: 'tween', interpolate: interpolatePath, ...tween.options } : undefined);
            ...
            $effect(() => {
                ...
                for (const seg of targets) {
                    if (seg.key === undefined)
                        continue;
                    active.add(seg.key);
                    if (seg.lineKey !== undefined)
                        lines.add(seg.lineKey);
                    ...
                    untrack(() => tweens.update(seg.key, seg.d, () => intoDrawnLine ? this.#collapsedPathData(seg.data) : this.#defaultPathData(seg.data)));
                }
                drawnLines = lines;
    ...
    yAccessor = $derived.by(() => {
        ...
        if (this.ctx.valueAxis === 'y') {
            ...
            if (this.seriesAccessor)
                return accessor(this.seriesAccessor);
        }
    ...
    zAccessor = $derived.by(() => {
        ...
        if (implied != null)
            return accessor(implied);
        if (props.data != null || this.series?.data != null)
            return null;
    ...
    #buildPath(resolvedData) {
        ...
        if (props.curve)
            path.curve(props.curve);
        return path(resolvedData) ?? '';
    }
    hasAnyStyleFn = $derived.by(() => {
        const p = this.#props;
        return (typeof p.stroke === 'function' ||
    ...
    d = $derived.by(() => {
        ...
        if ((this.hasAnyStyleFn || this.zAccessor) && !this.geo.projection)
            return '';
        ...
        if (this.geo.projection) {
            const coordinates = resolvedData
                .filter((d) => {
                if (props.defined)
                    return props.defined(d, 0, resolvedData);
    ...
    #segmentTargets = $derived.by(() => {
        if (!this.hasAnyStyleFn && !this.zAccessor)
            return null;
        const props = this.#props;
        if (this.geo.projection)
            return null;
        ...
        for (const lineData of this.lines) {
            ...
            if (this.hasAnyStyleFn) {
                ...
                out.push({
                    stroke: resolveColorProp(props.stroke, lineData[0], this.ctx.cScale) ??
    ...
    segments = $derived.by(() => {
        ...
        if (!targets || !tweens)
            return targets;
        return targets.map((seg) => seg.key === undefined ? seg : { ...seg, d: tweens.get(seg.key) ?? seg.d });
    });
    /**
    ...
    #isShown(d) {
        if (d == null || !this.zAccessor)
            return true;
        const key = this.zAccessor(d);
        if (!this.#namesSeries(key))
            return true;
    ...
    #defaultPathData(data) {
        const props = this.#props;
        if (!extractTweenConfig(props.motion))
            return '';
    ...
    #collapsedPathData(data) {
        const props = this.#props;
        if (!extractTweenConfig(props.motion) || data[0] == null)
            return '';
     ...
     */
    resolvedStroke = $derived(resolveColorProp(this.#props.stroke, this.lines[0]?.[0], this.ctx.cScale) ??
