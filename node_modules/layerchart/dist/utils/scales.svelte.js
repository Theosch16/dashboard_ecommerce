 ...
 * `series` colors.
 */
export function isScaleOrdinal(scale) {
    // Ordinal scales carry `unknown()`; continuous ones interpolate instead
    return (typeof scale.unknown === 'function' &&
        typeof scale.interpolate !== 'function' &&
 ...
 */
export function isScaleUtc(scale) {
    if (!isScaleTime(scale))
        return false;
    if (typeof scale.ticks !== 'function' || typeof scale.copy !== 'function')
        return false;
    const probe = scale
        .copy()
        .domain([new Date(Date.UTC(2024, 0, 1)), new Date(Date.UTC(2024, 0, 4))]);
    const ticks = probe.ticks(3);
    return (ticks.length > 0 &&
        ticks.every((tick) => tick.getUTCHours() === 0 && tick.getUTCMinutes() === 0 && tick.getUTCSeconds() === 0));
}
