 ...
 */
export function isWithinExtent(value, extent) {
    if (extent == null)
        return true;
    const min = extent[0];
    const max = extent[1];
 ...
 * ```
 */
export function isWithinSelection(selection, point) {
    if (selection == null)
        return true;
    return ((point.x === undefined || isWithinExtent(point.x, selection.x)) &&
