import { timeYear, timeDay, timeHour, timeMillisecond, timeMinute, timeSecond, timeTicks, utcDay, utcHour, utcMillisecond, utcMinute, utcSecond, utcYear, } from 'd3-time';
import { format, Duration, isLiteralObject, PeriodType, DateToken, } from '@layerstack/utils';
import { isScaleBand, isScaleTime, isScaleUtc } from './scales.svelte.js';
 ...
 */
export function filterTicksByFormat(tickVals, formatType, options = {}) {
    const utc = options.utc ?? false;
    const type = isLiteralObject(formatType) ? formatType.type : formatType;
    ...
    const onBoundary = (interval) => (val) => +interval.floor(val) === +val;
    if (type === 'integer') {
        return tickVals.filter(Number.isInteger);
    }
        ...
        return tickVals.filter((val) => (utc ? val.getUTCDate() : val.getDate()) < 7);
    }
...
export function autoTickFormat(options) {
    ...
    if (formatType) {
        ...
        if (isScaleUtc(scale)) {
            const config = isLiteralObject(formatType)
                ? formatType
                : { type: formatType };
            return (tick) => 
