...
import ChartGroupTestHarness from '../tests/ChartGroupTestHarness.svelte';
...
async function renderPair(options = {}) {
    ...
    render(ChartGroupTestHarness, {
        ...
        oncontext: (ctx, i) => (contexts[i] = ctx),
        ongroup: (g) => (contextGroup = g),
    });
    await vi.waitFor(() => {
    ...
    describe('pointer sharing', () => {
        ...
        it('lands in the right panel when the follower is faceted', async () => {
            ...
            await vi.waitFor(() => {
                ...
                const panel = chartB.facet.panels.find((p) => p.has(chartB.tooltip.data));
...
describe('brush slice', () => {
    ...
    async function renderBrushPair(options = {}) {
    ...
    describe('publishing from a gesture', () => {
        ...
        function pointerRun(el, from, to, y) {
            const types = from === to
                ? [
                    ['pointerdown', from],
                    ['pointerup', to],
        ...
        async function firstBrushEl() {
            let el = null;
            await vi.waitFor(() => {
                el = document.querySelectorAll('.lc-brush-context')[0];
...
describe('domain slice', () => {
    ...
    async function renderDomainPair(options = {}) {
