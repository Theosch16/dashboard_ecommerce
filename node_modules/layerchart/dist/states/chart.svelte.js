...
import { colorPropDataKey } from '../utils/dataProp.js';
...
function zoomForAxis(domain, from, to) {
    if (from == null || to == null)
        return undefined;
    if (typeof domain[0] === 'string') {
        // Categorical: the selection is a run of the domain, measured in indices
        const startIdx = domain.indexOf(from);
        const endIdx = domain.indexOf(to) + 1;
        const selected = endIdx - startIdx;
        if (selected <= 0 || domain.length === 0)
            return undefined;
    ...
    if (selectedRange <= 0 || baseRange <= 0)
        return undefined;
 ...
 */
export function transformForBrush(brush, axis, base) {
    ...
    if (axis === 'both') {
        ...
        if (y)
            translateY = -y.offset * base.height * x.scale;
    }
    return {
        scale: x.scale,
...
export class ChartState {
    ...
    cGroups = $derived.by(() => {
        ...
        const seriesNameThem = configured.length > 1 || (configured.length === 1 && configured[0].key !== 'default');
        if (seriesNameThem)
            return null;
    ...
    seriesLayout = $derived.by(() => {
        ...
        for (const value of [this.props.y, this.props.x]) {
            if (Array.isArray(value))
                return 'overlap';
            if (value != null && first != null && Array.isArray(accessor(value)(first)))
                return 'overlap';
        }
        ...
        if (valueOf == null && configured.length === 0)
            return 'overlap';
    ...
    isStacked = $derived.by(() => {
        if (this.seriesState.stackLayout == null)
            return false;
    ...
    stackAccessorsFor(options) {
        ...
        if (this.series.stackLayout == null)
            return null;
        if (this.seriesLayoutAuto && (!stacksImplicitly || ownData))
            return null;
        if (!seriesKey && !this.series.stacksByCategory)
            return null;
     ...
     */
    stackedValue(seriesKey, keyValue, value) {
        if (seriesKey == null || !this.isStacked)
            return value;
     ...
     */
    cKey = $derived.by(() => (this.cGroups ? (d) => this.c(d) : () => null));
    data = $derived.by(() => {
        ...
        return data.filter((d) => selected.isSelected(this.c(d)));
    });
    flatData = $derived.by(() => {
        ...
        const seen = new Set();
        for (const { info } of this._markInfos) {
            if (!info.data || seen.has(info.data))
                continue;
            seen.add(info.data);
    ...
    _xScaleProp = $derived.by(() => {
        ...
        return autoScale(this.brushXDomain ?? this.props.xDomain ?? this.groupXDomain, this.flatData, this.x);
    });
    _yScaleProp = $derived.by(() => {
        ...
        return autoScale(this.brushYDomain ?? this.props.yDomain ?? this.groupYDomain, this.flatData, this.y);
    });
     ...
     */
    facetBand = $derived.by(() => this.facetState.enabled && this.tooltip.mode === 'facet' && this.seriesState.isDefaultSeries);
     ...
     */
    #xSeriesValues = $derived(this.#getAxisSeriesValues('x'));
    #ySeriesValues = $derived(this.#getAxisSeriesValues('y'));
    resolveDomain(axis) {
    ...
    #transformDomain(axis, at) {
        ...
        if (transform?.mode !== 'domain')
            return base;
        if (transform.axis !== axis && transform.axis !== 'both')
            return base;
        if (!(size > 0))
            return base;
    ...
    #visibleSubBandDomain(domain) {
        ...
        return domain.filter((key) => !seriesKeys.has(key) || visibleKeys.has(key));
    }
    x1Domain = $derived.by(() => {
    ...
    cScale = $derived.by(() => {
        ...
        return this.seriesState?.cScale ?? null;
    });
    ...
    get brush() {
        return this.brushState ?? ChartState.#fallbackBrush;
    }
    get transform() {
