...
const transformProps = { mode: 'domain', axis: 'x', scaleExtent: [1, 50] };
...
describe('ChartGroup + transform', () => {
    it('settles with an animated transform', async () => {
        ...
        expect(+group.domain.x[0]).toBe(+range[0]);
        expect(+group.domain.x[1]).toBe(+range[1]);
    });
    it('publishes the domain a zoomed chart settles on', async () => {
        ...
        await vi.waitFor(() => {
            expect(+plain.xDomain[1]).toBe(+group.domain.x[1]);
        });
    });
    it('zooms a transform chart to a domain shared by another', async () => {
        ...
        await vi.waitFor(() => {
            ...
            const settled = zoomable._targetXDomain;
    ...
    it('settles rather than trading updates between the two', async () => {
        ...
        expect(+group.domain.x[0]).toBe(+settled.x[0]);
        expect(+group.domain.x[1]).toBe(+settled.x[1]);
    });
});
