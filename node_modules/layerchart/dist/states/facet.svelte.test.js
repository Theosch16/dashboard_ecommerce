...
function renderChart(extra, componentProps = {}) {
    let ctx = null;
    render(TestHarness, {
        component: Circle,
        chartProps: {
...
describe('facets', () => {
    describe('layout', () => {
        ...
        it('partitions the rows across panels', async () => {
            ...
            expect(ctx().facet.panels[0].data.every((d) => d.g === 'a')).toBe(true);
        });
    ...
    describe('headers', () => {
        it('draws the panel headers from the default layout', async () => {
            ...
            const labels = Array.from(document.querySelectorAll('.lc-facet-axis-x text')).map((t) => t.textContent?.trim());
        ...
        it('draws the row headers when faceting on `fy`', async () => {
            ...
            const labels = Array.from(document.querySelectorAll('.lc-facet-axis-y text')).map((t) => t.textContent?.trim());
    ...
    describe('interaction', () => {
        ...
        function renderPaired(chartProps = {}, componentProps = {}) {
            let ctx = null;
            render(TestHarness, {
                component: Circle,
                chartProps: {
        ...
        async function hoverPoint(ctx, panelIndex, v, w) {
            const hitArea = document.querySelector('.lc-tooltip-context');
            const rect = hitArea.getBoundingClientRect();
            const panel = ctx.facet.panels[panelIndex];
            const eventInit = {
                bubbles: true,
                clientX: rect.x + panel.x + ctx.xScale(v),
                clientY: rect.y + panel.y + ctx.yScale(w),
            };
        ...
        it('renders a tooltip per panel with `facetAll`, each showing its own row', async () => {
            ...
            const hitArea = (await vi.waitUntil(() => document.querySelector('.lc-tooltip-context')));
            await vi.waitFor(() => {
                const rect = hitArea.getBoundingClientRect();
                const eventInit = {
                    bubbles: true,
                ...
                const text = Array.from(document.querySelectorAll('.lc-tooltip-root')).map((t) => t.textContent?.replace(/\s+/g, ' ').trim());
        ...
        describe('panel name', () => {
            ...
            function label(facet) {
                const ctx = renderChart({ data: named, fx: 'g', ...(facet ? { facet } : null) });
                return (row) => ctx().facet.tooltipLabel(row);
            }
            ...
            it('comes off the row with a `tooltip` function', () => {
                ...
                expect(label({ tooltip: (d) => d.name })(named[0])).toBe('North');
            });
             ...
             */
            async function headerText(facet) {
                render(BarChart, {
                    props: {
                        data: named,
                        x: 'm',
            ...
            it('reaches the tooltip header through a `tooltip` function', async () => {
                expect(await headerText({ tooltip: (d) => d.name })).toContain('North · Jan');
            });
            it('names each panel in its own tooltip with `facetAll`', async () => {
                ...
                await vi.waitFor(() => {
                    ...
                    hitArea.dispatchEvent(new PointerEvent('pointerenter', eventInit));
                    hitArea.dispatchEvent(new PointerEvent('pointermove', eventInit));
                    const headers = Array.from(document.querySelectorAll('.lc-tooltip-header')).map((h) => h.textContent?.replace(/\s+/g, ' ').trim());
