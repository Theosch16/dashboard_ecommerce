...
describe('ChartState facetBand', () => {
    ...
    function createFaceted(props, mode = 'facet') {
        const created = createChartState({
            data: facetData,
            x: 'year',
            xScale: scaleBand(),
            y: 'percent',
        ...
        created.state.tooltipState = new TooltipState(mode, () => { }, () => { });
        flushSync();
        return created;
    }
...
describe('ChartState zoomToBrush', () => {
    ...
    const half = [new Date(2024, 0, 1), new Date(2024, 0, 16)];
    const brush = { x: half, y: [null, null] };
    ...
    function transformStub() {
        ...
        return {
            ...
            state: {
                ...
                scale: 1,
                translate: { x: 0, y: 0 },
                setScale: (v) => scales.push(v),
                setTranslate: (p) => translates.push(p),
            },
        };
    }
    it('should apply the zoom when transform state already exists', () => {
        const { state, cleanup } = createChartState({ data, x: 'date', y: 'value' });
        const stub = transformStub();
        try {
            state.transformState = stub.state;
    ...
    it('should keep the range anchored when the scale clamps', () => {
        ...
        try {
            ...
            state.transformState = { ...stub.state, targetScale: 4 };
            state.zoomToBrush({ x: [new Date(2024, 0, 10), new Date(2024, 0, 13)], y: [null, null] }, 'x');
...
describe('ChartState `c` legend key', () => {
    ...
    function createCategoryChart(props = {}) {
...
describe('ChartState implied colour channel', () => {
    ...
    it('takes the colour column from a mark that named one', () => {
        ...
        const { state, cleanup } = createChartState({ data, x: 'date', y: 'value', cRange });
        try {
    ...
    it('keeps a category on the same colour whatever else is in the data', () => {
        ...
        const colourOf = (rows, island, withMark) => {
            const { state, cleanup } = createChartState({ data: rows, x: 'date', y: 'value', cRange }); // prettier-ignore
            try {
                if (withMark)
                    state.registerMark({ x: 'date', y: 'value', color: 'island' });
                flushSync();
