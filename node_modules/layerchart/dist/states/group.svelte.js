...
import { findDatumByValue } from '../utils/tooltip.js';
...
export class ChartGroupState {
    ...
    join(id) {
        if (this.#memberIds.has(id)) {
            ...
            console.warn(`[layerchart] Two charts joined a ChartGroup with id "${String(id)}" — they can't be told apart as a pointer/brush source, so updates between them are ignored. Give each chart a unique \`id\`.`);
        }
        this.#memberIds.add(id);
        return () => this.#memberIds.delete(id);
    }
    /** Shared visible domain.  `$state.raw` for the same reasons as `pointer`. */
    domain = $state.raw(emptyDomain());
    ...
    setPointer(pointer) {
        ...
        if (this.pointer.active && samePosition(this.pointer, next))
            return;
        this.pointer = next;
    }
    /**
     ...
     */
    clearPointer(source = null) {
        if (!this.pointer.active)
            return;
        if (source != null && this.pointer.source !== source)
            return;
    ...
    setBrush(brush) {
        ...
        if (this.brush.active && sameSelection(this.brush, next))
            return;
        this.brush = next;
    }
    /**
     ...
     * ```
     */
    setHighlight(key, source = null) {
        if (this.series.highlightKey === key && this.series.highlightSource === source)
            return;
     ...
     */
    clearHighlight(source = null) {
        if (this.series.highlightKey == null)
            return;
        if (source != null && this.series.highlightSource !== source)
            return;
    ...
    setHidden(keys, source = null) {
        ...
        if (sameKeys(this.series.hiddenKeys, keys))
            return;
     ...
     */
    clearHidden(source = null) {
        if (this.series.hiddenKeys.length === 0)
            return;
    ...
    setDomain(domain) {
        ...
        if (this.domain.active && sameDomain(this.domain, next))
            return;
        this.domain = next;
    }
    /**
 ...
 */
function optedInto(option, slice) {
...
function fromPercent(scale, range, percent) {
    const lo = min(range);
    const hi = max(range);
 ...
 * and scales.
 */
export function resolvePointerData(pointer, ctx, options) {
 ...
 */
export function connectToChartGroup(ctx, getGroup, getMemberOptions = () => undefined) {
    ...
    $effect(() => {
        ...
        return () => {
            ...
            if (tooltip.onChange === publish)
                tooltip.onChange = undefined;
        };
    });
    ...
    $effect(() => {
        ...
        if (!group || !seriesOptions?.highlight)
            return;
        if (!allows(getMemberOptions()?.publish, 'series'))
            return;
        const series = ctx.series;
        const publish = () => {
            // Ignore a highlight the group applied — it would echo straight back out
            if (series.highlightSource != null)
                return;
        ...
        return () => {
            // Only release our own handler — a re-run may already have installed a newer one
            if (series.onHighlightChange === publish)
                series.onHighlightChange = undefined;
        };
    });
    ...
    $effect(() => {
        ...
        if (!group || !seriesOptions?.highlight)
            return;
        if (!allows(getMemberOptions()?.subscribe, 'series'))
            return;
        const shared = group.series;
        if (shared.highlightSource === ctx.id)
            return; // this chart published it
        ...
        untrack(() => {
            if (ctx.series.highlightKey === shared.highlightKey)
                return;
            ctx.series.setHighlight(shared.highlightKey, 
    ...
    $effect(() => {
        const group = getGroup();
        const seriesOptions = group?.seriesOptions;
        if (!group || !seriesOptions?.visibility)
            return;
        if (!allows(getMemberOptions()?.publish, 'series'))
            return;
    ...
    $effect(() => {
        ...
        if (!allows(getMemberOptions()?.subscribe, 'series'))
            return;
        const shared = group.series;
        if (shared.visibilitySource === ctx.id)
            return; // this chart published it
        const keys = legendKeys(ctx);
        ...
        untrack(() => {
            ...
            if (sameKeys(selectedKeys.current, next))
                return;
    ...
    $effect(() => {
        ...
        if (!group)
            return;
        return () => {
            if (group.pointer.source === ctx.id)
                group.clearPointer();
            if (group.brush.source === ctx.id)
                group.clearBrush();
            if (group.domain.source === ctx.id)
                group.clearDomain();
            if (group.series.highlightSource === ctx.id)
    ...
    $effect(() => {
        ...
        if (!group || !domainOptions)
            return;
        if (!allows(getMemberOptions()?.subscribe, 'domain'))
            return;
        const shared = group.domain;
        if (shared.source === ctx.id)
            return; // this chart published it
        untrack(() => {
        ...
        if (untrack(() => ctx.transformState?.mode === 'domain')) {
            untrack(() => {
                if (shared.active) {
                    following = true;
                    ctx.zoomToBrush({ x: (shared.x ?? [null, null]), y: (shared.y ?? [null, null]) }, domainOptions.axis);
                }
                else if (following) {
        ...
        ctx.groupXDomain = domainOptions.axis !== 'y' ? shared.x : undefined;
        ctx.groupYDomain = domainOptions.axis !== 'x' ? shared.y : undefined;
    });
    /**
    ...
    $effect(() => {
        ...
        if (!group || !domainOptions)
            return;
        if (!allows(getMemberOptions()?.publish, 'domain'))
            return;
        if (ctx.transformState?.mode !== 'domain')
            return;
        const x = domainOptions.axis !== 'y' ? ctx._transformTargetXDomain : undefined;
        const y = domainOptions.axis !== 'x' ? ctx._transformTargetYDomain : undefined;
        untrack(() => {
            const shared = group.domain;
            // Back to the full extent — no shared domain, rather than one covering everything
            if (ctx.transform.targetScale === 1) {
                if (shared.active && shared.source === ctx.id)
                    group.clearDomain(ctx.id);
                return;
            }
            const unchanged = isEqualValue(x?.[0], shared.x?.[0]) &&
    ...
    $effect(() => {
        const group = getGroup();
        const pointerOptions = group?.pointerOptions;
        if (!group || !pointerOptions)
            return;
        if (!allows(getMemberOptions()?.subscribe, 'pointer'))
            return;
        const pointer = group.pointer;
        if (pointer.source === ctx.id)
            return; // this chart published it
        if (!pointer.active) {
            ctx.tooltip.hide();
            return;
    ...
    return {
        ...
        publishBrush(brush) {
            ...
            if (group.domainOptions && optedInto(publish, 'domain')) {
                if (brush.active) {
                    group.setDomain({ x: brush.x, y: brush.y, source: ctx.id });
                }
                else {
                    group.clearDomain(ctx.id);
                }
        ...
        publishDomain(x, y) {
            ...
            if (x == null && y == null) {
                ...
                group.setDomain({ x: x, y: y, source: ctx.id });
            }
            releaseAppliedDomain();
        },
    };
